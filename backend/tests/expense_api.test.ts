...
describe("Tests for /api/expense", () => {
  describe("Tests for GET /api/expenses/:yearMonth", () => {
    describe("", () => {
      test("GET /api/expenses/:yearMonth does not work with invalid data", async () => {
        ...
        response = await api.get("/api/expenses/2022-7");
        expect(response.status).toBe(400);
        expect(response.body.error).toBe(
          "Expecting a valid date in form YYYY-MM"
        );
    ...
    describe("Test for GET/api/expenses/2023-07", () => {
      beforeEach(async () => {
      ...
      test("Able to get months data ", async () => {
        const response = await api.get("/api/expenses/2023-07");
        expect(response.status).toBe(200);
        expect(response.body.totalAmount).toBe(50);
        expect(response.body.data[0].category).toBe("Gas");
